...
def transform_load_data(task_instance):
    data = task_instance.xcom_pull(task_ids="group_a.tsk_extract_bangkok_weather_data")
...
def save_joined_data_s3(task_instance):
    ...
    df.to_csv(f"s3://airflow-parallel-eirs-yml/{dt_string}.csv", index=False)
...
with DAG('weather_dag_2',
        ...
        with TaskGroup(group_id = 'group_a', tooltip= "Extract_from_S3_and_weatherapi") as group_A:
            ...
            uploadS3_to_postgres  = PostgresOperator(
                ...
                postgres_conn_id = "postgres_conn",
                sql = "SELECT aws_s3.table_import_from_s3('city_look_up', '', '(format csv, DELIMITER '','', HEADER true)', 'airflow-parallel-eirs-yml', 'city.csv', 'ap-southeast-1');"
            )
            ...
            is_bangkok_weather_api_ready = HttpSensor(
                task_id ='tsk_is_bangkok_weather_api_ready',
                http_conn_id='weathermap_api',
                endpoint='/data/2.5/weather?q=bangkok&APPID=d72300bd5b24931f5ad29c6119923aa6'
            )

            extract_bangkok_weather_data = SimpleHttpOperator(
                task_id = 'tsk_extract_bangkok_weather_data',
                http_conn_id = 'weathermap_api',
                endpoint='/data/2.5/weather?q=bangkok&APPID=d72300bd5b24931f5ad29c6119923aa6',
                method = 'GET',
            ...
            transform_load_bangkok_weather_data = PythonOperator(
                task_id= 'transform_load_bangkok_weather_data',
            ...
            create_table_2 >> is_bangkok_weather_api_ready >> extract_bangkok_weather_data >> transform_load_bangkok_weather_data >> load_weather_data
